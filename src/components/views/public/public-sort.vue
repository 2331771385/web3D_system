<template>
    <div class="tableContainer">
        <div class="table-label">
            ...
            <span class="pageTitle">服务分类列表</span>
        </div>
        <!-- 搜索框 -->
        <div class='searchInput'>
            <div class="search-top">
                ...
                <Input style="width:260px" 
                    v-model="search"
                    placeholder="服务分类名称关键字"
                    clearable
                    @on-enter="clickSub"
                    @on-clear='getPublicSortList'
                    @on-change='getPublicSortList'
                />
                <Button type="primary" @click="getPublicSortList" icon="ios-search">查找</Button>
                <Button type="success" @click="addInfo" icon='ios-add'>新增</Button>
            </div>
        </div>

        <!-- 列表数据展示 -->
        <Table border :columns='columns' :data='publicSortList'>
            <!-- 创建时间 -->
        ...
        <!-- 新增数据 -->
        <el-dialog  title="添加分类类型" :visible.sync="addVisible" width="480px">
            <el-form :model="form" ref="form" label-width="115px" :rules='ruleValidate' class="demo-ruleForm">
                <el-form-item label="类型名称:" prop="serviceTypeName">
                    <el-input 
        ...
        <!-- 修改操作 -->
        <el-dialog  title="修改分类类型" :visible.sync="updateVisible" width="480px">
...
export default {
    ...
    methods: {
        ...
        addSuccess(name) {
            this.$refs[name].validate(valid => {
                if (valid) {
                    ...
                    formData.append('serviceTypeName', this.form.serviceTypeName);
                    formData.append('sortIndex', this.form.sortIndex);
                    formData.append('token', window.localStorage.getItem('Authorization'));

                    axios({
                        url: this.$store.state.UrlIP + '/publicService/insertTypeData',
                        method: 'post',
                        data: formData,
                        headers: {
        ...
        updateSuccess(name) {
            this.$refs[name].validate(valid => {
                if (valid) {
                    ...
                    formData.append('sortIndex', this.form.sortIndex);
                    formData.append('token', window.localStorage.getItem('Authorization'));
                    formData.append('serviceTypeId', this.msg.serviceTypeId);

                    axios({
                        url: this.$store.state.UrlIP + '/publicService/updateTypeData',
                        method: 'post',
                        data: formData,
                        headers: {
                            'Content-type':'multipart/form-data'
        ...
        deleteInfo(row, index) {
            this.$Modal.confirm({
                title: '注意',
                content: '是否删除公共分类及其相关信息？',
                onOk: () => {
                    this.handleDelete(row.serviceTypeId);
                },
                onCancel: () => {
